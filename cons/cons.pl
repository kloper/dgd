...
sub KeepPrecious {
    my($env, @parm) = @_;
    map($_->{precious} = 1, map($dir::cwd->lookup($env->_subst($_)), @parm));
}
...
sub AfterBuild {
    ...
    my $file;
    for $file (map($dir::cwd->lookup($env->_subst($_)), @_)) {
...
sub find {
	...
	@$cpppath = grep( ($_ and ($_ ne "")), @{$cpppath} );
